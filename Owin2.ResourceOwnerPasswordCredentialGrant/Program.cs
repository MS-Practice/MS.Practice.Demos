...
namespace Owin2.ResourceOwnerPasswordCredentialGrant
{
    class Program
    {
        ...
        private static void InitializeWebServerClient()
        {
            ...
            var authorizationServert = new AuthorizationServerDescription
            {
                AuthorizationEndpoint = new Uri(authorizationServerUri,Paths.AuthorizePath),
                TokenEndpoint = new Uri(authorizationServerUri,Paths.TokenPath)
            };

            _webServerClient = new WebServerClient(authorizationServert, Clients.Client1.Id, Clients.Client1.Secret);
        }

        private static void RequestToken()
        {
            var state = _webServerClient.ExchangeUserCredentialForToken("UserName", "Password", scopes: new[] { "bio" });
